...
public static class Recursion
{
    ...
    public static int SumSquaresRecursive(int n)
    {
        ...
        // Recursive case: return n^2 + SumSquaresRecursive(n - 1)
    ...
    {
        // Base case: if the size is 0, add the current word to results
        if (size == 0)
        {
            results.Add(word);
            return;
        ...
        // Recursive case: iterate through each letter and call the function recursively
        for (int i = 0; i < letters.Length; i++)
        {
            // Choose the current letter and append it to the word
            ...
            // Recur with the remaining letters and reduced size
            PermutationsChoose(results, remainingLetters, size - 1, newWord);
        }
    }
    ...
    public static void WildcardBinary(string pattern, List<string> results)
    {
        ...
        // Recursive case: find the first wildcard and replace it with '0' and '1'
